...
class ZModel():
    ...
    def dumpSave(self, fpath=None): 
        ...
        try:           
            with open( fpath, "wb") as fd:
                ...
                zlogger.log("{}.model.dump".format(self.__class__), "Model saved to file successfully - {}".format(fpath) )
        except:
    ...
    def preprocessText(self, input_text): 
        ...
        if len(input_text) == 1 and len(input_text[0].strip() ) == 0 : 
            return ['define' ]
    ...
    ################## HELPERS #################
    def getModelFPath(self, fpath=None):         ## TODO: make sense of previously save fpath 
        if self.model_fpath is None:
            model_fpath = self.getClassName() if self.name is None else self.name         
            self.model_fpath = f"{model_fpath}.zmd" 

        if fpath is not None:
            self.model_fpath = f"{fpath}.zmd"
