...
def cleanup_stopwords(text, stop_wordz=None):        
    # zlogger.log('cleanup_stopwords', "IN.STOPZ = {}".format( ", ".join(stop_wordz ) ) )  
    stopz = stopwords.words('english') if stop_wordz is None else stop_wordz 
    stopz = set( stopz )  
    # zlogger.log('cleanup_stopwords', "STOPZ = {}".format( ", ".join(stopz ) ) )  
...
class ZDataset():
    ...
    def updateXIndex(self):
        the_data = self.data if self.clean_data is None else self.clean_data 
        self.x_index = { i : x for i, x in enumerate(the_data) } if the_data is not None else None 
    ...
    def thresholdRatio(self):        
        ...
        n_observations = len( the_data ) 
        avg_words_per_observation = [ len( nltk.word_tokenize(rec) )  for rec in the_data]
